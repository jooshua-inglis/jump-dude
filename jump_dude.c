...
bool make_column( int column, int rows, int *safe_blocks, int *danger_blocks, game_sprite blocks[columns][rows] ) {
    ...
    do {
        ...
        for (int row = 0; row < rows ; row ++){
            ...
            else if (*safe_blocks + row_safe_blocks > 160) block_type = 2;
            else block_type = rand_type();

            width = rand() % 6 + 5;

            if (block_type == 1) row_safe_blocks ++;
            else if (block_type == 2) row_danger_blocks += 1;
...
void draw_game( int score, int lives, int time, sprite_id jump_dude, sprite_id treasure, game_sprite blocks[columns][rows] ){
    clear_screen();
    char time_message[100];
    get_time_message(time_message, time);
...
void blocks_step( game_sprite blocks[columns][rows]) {
    for (int column = 0; column < columns; column ++) {
        for ( int row = 1; row < rows -1; row ++) {
            ...
            if (sprite->x < 0 && sprite->dx < 0) sprite->x = screen_width();
...
game_sprite get_collisions(bool *down_colide, bool *up_colide, bool *horizontal_colide, sprite_id jump_dude, game_sprite blocks[columns][rows]) {
...
void colision( sprite_id jump_dude, sprite_id treasure,  int *lives, int *score, bool *air_born, bool *down_colide, game_sprite blocks[columns][rows], game_sprite *colided_block ) {
...
void cleanup( sprite_id jump_dude, sprite_id treasure, game_sprite blocks[columns][rows], timer_id *game_timer, timer_id *treasure_timer ) {
